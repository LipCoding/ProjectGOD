...
bool CResourcesManager::Init()
{
	...
	// 구 메쉬를 생성한다.
	pMesh = CreateSphere("Sphere", 1.f, 2);
...
CTexture * CResourcesManager::LoadTexture_Dynamic(const string & strKey, const char * pFileName, const string & strPathKey)
{
	CTexture*	pTexture = FindTexture(strKey);
	...
	if (!pTexture->LoadTexture_Dynamic(strKey, pFileName, strPathKey))
	{
		SAFE_RELEASE(pTexture);
		return NULL;
	}
