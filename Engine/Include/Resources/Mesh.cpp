...
string CMesh::GetShadowShaderKey() const
{
	return m_strShadowShaderKey;
}
	...
	const Vector3& vLandPos)
{
	...
	pNaviMesh->CreateGridMapAdj(512);
}
...
float CMesh::AngleFromXY(float x, float y)
{
	...
	if (x >= 0.0f)
	{
		...
		if (theta < 0.0f)
			theta += 2.0f*PG_PI; // in [0, 2*pi).
	}

	// Quadrant II or III
	else
		theta = atanf(y / x) + PG_PI; // in [0, 2*pi).
	...
	for (iter = pvecContainer->begin(); iter != iterEnd; ++iter)
	{
		...
		if ((*iter)->bBump)
		{
			if ((*iter)->bAnimation)
			{
				...
				m_strShadowShaderKey = STANDARD_SHADOWMAP_BUMPANIM_SHADER;
			...
			else
			{
				...
				m_strShadowShaderKey = STANDARD_SHADOWMAP_BUMP_SHADER;
		...
		else
		{
			if ((*iter)->bAnimation)
			{
				...
				m_strShadowShaderKey = STANDARD_SHADOWMAP_TEXNORMALANIM_SHADER;
			...
			else
			{
				...
				m_strShadowShaderKey = STANDARD_SHADOWMAP_TEXNORMAL_SHADER;
...
bool CMesh::LoadFromFullPath(const char * pFullPath)
{
	...
	if (m_strShaderKey == STANDARD_BUMP_ANIM_SHADER)
		...
		m_strShadowShaderKey = STANDARD_SHADOWMAP_COLOR_SHADER;

	else if (m_strShaderKey == STANDARD_COLOR_NORMAL_SHADER)
		m_strShadowShaderKey = STANDARD_SHADOWMAP_COLORNORMAL_SHADER;
