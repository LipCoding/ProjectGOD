	...
	CComponent(animation)
{
	...
	for (size_t i = 0; i < animation.m_vecBones.size(); ++i)
	{
		...
		*pBone->matBone = *animation.m_vecBones[i]->matBone;
		...
		list<CBoneSocket*>::const_iterator	iterBEnd = animation.m_vecBones[i]->SocketList.end();

		for (iterB = animation.m_vecBones[i]->SocketList.begin(); iterB != iterBEnd; ++iterB)
		{
...
{
	PBONE	pBone = FindBone(strBoneName);
...
bool CAnimation::SetSocketTarget(const string & strBoneName,
	const string & strSocketName, CGameObject * pTarget)
{
	PBONE	pBone = FindBone(strBoneName);
...
int CAnimation::Update(float fTime)
{
	...
	else
	{
		...
		// 나중에 수정하고 다시 만들어야됨.
		//m_fClipProgress = m_fClipProgress > 1.f ? 1.f : m_fClipProgress;

		//// Callback 이 있는지 체크한다.
		//for (size_t i = 0; i < m_vecChannel[m_iCurChannel].pClip->m_tInfo.vecCallback.size();
		//	++i)
		//{
		//	if (m_vecChannel[m_iCurChannel].pClip->m_tInfo.vecCallback[i]->fAnimationProgress >=
		//		m_fClipProgress && !m_vecChannel[m_iCurChannel].pClip->m_tInfo.vecCallback[i]->bCall)
		//	{
		//		m_vecChannel[m_iCurChannel].pClip->m_tInfo.vecCallback[i]->bCall = true;
		//		m_vecChannel[m_iCurChannel].pClip->m_tInfo.vecCallback[i]->func(fTime);
		//	}
		//}
...
{
	Matrix	matWorld;
	matWorld = m_pTransform->GetLocalMatrix() * m_pTransform->GetWorldMatrix();
	...
	for (size_t i = 0; i < m_vecBones.size(); ++i)
	{
		...
		{
			(*iter)->Update(matWorld, *m_vecBones[i]->matBone);
		}
	}
	return 0;
}
