...
void CMaterial::AddMultiTexture(const string & strSmpKey, 
	ID3D11ShaderResourceView * pSRV, int iRegTex, int iRegSmp)
{
...
void CMaterial::SetMaterial(int iShaderConstantType)
{
	...
	if (m_pDifInfo)
	{
		if (m_pDifInfo->pTexture != nullptr)
		{
	...
	if (m_pNormalInfo)
	{
		if (m_pNormalInfo->pTexture != nullptr)
		{
	...
	if (m_pSpecularInfo)
	{
		if (m_pSpecularInfo->pTexture != nullptr)
		{
...
void CMaterial::ResetMaterial(int iShaderConstantType)
{
	...
	if (m_pDifInfo)
	{
		...
		{
			if (iShaderConstantType & SCT_VERTEX)
				CONTEXT->VSSetSamplers(m_pDifInfo->iRegisterSmp, 1, &pSmp);

			if (iShaderConstantType & SCT_PIXEL)
				CONTEXT->PSSetSamplers(m_pDifInfo->iRegisterSmp, 1, &pSmp);
		}*/
	}
	...
	if (m_pNormalInfo)
	{
		...
		{
			if (iShaderConstantType & SCT_PIXEL)
				CONTEXT->PSSetSamplers(m_pNormalInfo->iRegisterSmp, 1, &pSmp);
		}*/
	}
	...
	if (m_pSpecularInfo)
	{
		...
		{
			if (iShaderConstantType & SCT_PIXEL)
				CONTEXT->PSSetSamplers(m_pSpecularInfo->iRegisterSmp, 1, &pSmp);
		}*/
	}
