...
PKEYFRAME CAnimationClip::GetKeyFrame(int iBone, int iFrame) const
{
	...
	if (m_vecKeyFrame[iBone]->vecKeyFrame.size() > iFrame)
	{
		temp = m_vecKeyFrame[iBone]->vecKeyFrame.at(iFrame);
	}
	else
		temp = m_vecKeyFrame[iBone]->vecKeyFrame[m_vecKeyFrame[iBone]->vecKeyFrame.size() - 1];
