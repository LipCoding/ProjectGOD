...
CAnimation2D::CAnimation2D(const CAnimation2D & anim)	:
	CComponent(anim)
{
	m_bRenderer2D = anim.m_bRenderer2D;
...
void CAnimation2D::SetRenderer2DEnable(bool bRenderer)
{
	m_bRenderer2D = bRenderer;
}
...
bool CAnimation2D::ChangeAnimation(const string & strKey)
{
	...
	if (m_bRenderer2D)
	{
		...
		if (pRenderer)
		{
			...
			if (pClip->eType == A2D_FRAME)
				pMaterial->SetDiffuseTexRegister(10);

			else
				pMaterial->SetDiffuseTexRegister(0);
