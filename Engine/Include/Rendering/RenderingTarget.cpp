...
ID3D11DepthStencilView * CRenderingTarget::GetDepthStencilView() const
{
	return m_pDepthView;
}
...
void CRenderingTarget::SetTarget()
{
	...
	CONTEXT->OMGetRenderTargets(1, &m_pOldTargetView, &m_pOldDepth);

	ID3D11DepthStencilView*	pDepthView = m_pOldDepth;

	if (m_pDepthView)
		pDepthView = m_pDepthView;

	CONTEXT->OMSetRenderTargets(1, &m_pTargetView, pDepthView);
}
...
void CRenderingTarget::ResetTarget()
{
	...
	CONTEXT->OMSetRenderTargets(1, &m_pOldTargetView, m_pOldDepth);
	SAFE_RELEASE(m_pOldDepth);
	SAFE_RELEASE(m_pOldTargetView);
}
	...
	const string & strPathKey)
{
	...
	ScratchImage	img;
	CaptureTexture(DEVICE, CONTEXT, m_pTargetTex, img);
...
void CRenderingTarget::ResetShader(int iRegister)
{
	...
	CONTEXT->PSSetShaderResources(iRegister, 1, &pView);
}
