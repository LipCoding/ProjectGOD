...
cbuffer ParticleCBuffer	: register(b11)
{
	...
	float   g_vAlpha;
	float3  g_vParticleEmpty4;
	matrix  g_mRot;
	float   g_fAngle;
	float3  g_vParticleEmpty5;
}
...
{
	float2 sc;
	sincos((theta / 180.f * 3.141592f), sc.x, sc.y);
	...
	inout TriangleStream<GS_OUTPUT_PARTICLE> TriStream)
{
	...
	float4 pos = float4(input[0].vCenter, 1.f);
	//pos = mul(pos, g_mRot);

	float4 axis_x = g_mRot[0];
	float4 axis_y = g_mRot[1];

	vPos[0] = float4(pos - g_vCameraAxisX * fHalfX -
		g_vCameraAxisY * fHalfY, 1.f);
	vPos[1] = float4(pos - g_vCameraAxisX * fHalfX +
		g_vCameraAxisY * fHalfY, 1.f);
	vPos[2] = float4(pos + g_vCameraAxisX * fHalfX -
		g_vCameraAxisY * fHalfY, 1.f);
	vPos[3] = float4(pos + g_vCameraAxisX * fHalfX +
		g_vCameraAxisY * fHalfY, 1.f);

	/*vRotPos = mul(vRotPos, g_mRot);*/

	GS_OUTPUT_PARTICLE   output = (GS_OUTPUT_PARTICLE)0;

	float2   vUV[4];
	//float2 rotateUVs(float2 TexCoords, float2 center, float theta)
	vUV[0] = rotateUVs(float2(0.f, 1.f), float2(0.5f, 0.5f), g_fAngle);
	vUV[1] = rotateUVs(float2(0.f, 0.f), float2(0.5f, 0.5f), g_fAngle);
	vUV[2] = rotateUVs(float2(1.f, 1.f), float2(0.5f, 0.5f), g_fAngle);
	vUV[3] = rotateUVs(float2(1.f, 0.f), float2(0.5f, 0.5f), g_fAngle);
