...
// Bone
typedef struct PG_DLL _tagFbxBone
{
	string	strName;
	int		iDepth;
	int		iParentIndex;
...
// Key Frame
typedef	struct PG_DLL _tagFbxKeyFrame
{
...
// Bone Key Frame
typedef	struct PG_DLL _tagFbxBoneKeyFrame
{
	int		iBoneIndex;
...
// Fbx Animation Clip
typedef struct PG_DLL _tagFbxAnimationClip
{
	string			strName;
	FbxTime			tStart;
	FbxTime			tEnd;
...
typedef struct PG_DLL _tagFbxMaterial
{
	Vector4		vDif;
	Vector4		vAmb;
	Vector4		vSpc;
...
typedef struct PG_DLL _tagFbxWeight
{
	int		iIndex;
	double	dWeight;
}FBXWEIGHT, *PFBXWEIGHT;

typedef struct PG_DLL _tagFBXMeshContainer
{
