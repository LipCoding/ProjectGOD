...
typedef struct PG_DLL _tagResolution
{
	UINT	iWidth;
	UINT	iHeight;
...
// Color Vertex
typedef struct PG_DLL _tagVertexColor
{
	Vector3	vPos;
	Vector4	vColor;
...
// Color Normal Vertex
typedef struct PG_DLL _tagVertexColorNormal
{
	Vector3	vPos;
	Vector3	vNormal;
	Vector4	vColor;
...
// Tex Vertex
typedef struct PG_DLL _tagVertexTex
{
	Vector3	vPos;
	Vector2	vUV;
...
// Tex Normal Vertex
typedef struct PG_DLL _tagVertexTexNormal
{
	Vector3	vPos;
	Vector3	vNormal;
	Vector2	vUV;
...
// Bump Vertex
typedef struct PG_DLL _tagVertexBump
{
	Vector3	vPos;
	Vector3	vNormal;
	Vector2	vUV;
...
// Bump Vertex
typedef struct PG_DLL _tagVertexAnim
{
	Vector3	vPos;
	Vector3	vNormal;
	Vector2	vUV;
...
// Bump Vertex
typedef struct PG_DLL _tagVertexBumpAnim
{
	Vector3	vPos;
	Vector3	vNormal;
	Vector2	vUV;
...
typedef struct PG_DLL _tagAnimation2DCBuffer
{
	int		iType;
	int		iFrameX;
	int		iFrameY;
...
typedef struct PG_DLL _tagButtonCBuffer
{
	Vector4	vColor;
	float	fLight;
	Vector3	vEmpty;
...
// Rect Info
typedef struct PG_DLL _tagRectInfo
{
	float	l;
	float	t;
	float	r;
...
typedef struct PG_DLL _tagObb2DInfo
{
	Vector3	vCenter;
	Vector3	vAxisX;
	Vector3	vAxisY;
...
typedef struct PG_DLL _tagAABB
{
	Vector3	vMin;
	Vector3	vMax;
}AABB, *PAABB;

typedef struct PG_DLL _tagMaterial
{
	Vector4	vDif;
	Vector4	vAmb;
	Vector4	vSpc;
...
typedef struct PG_DLL _tagLight
{
	Vector4	vDif;
	Vector4	vAmb;
	Vector4	vSpc;
...
typedef struct PG_DLL _tagSphere
{
	Vector3	vCenter;
	float	fRadius;
}SPHERE, *PSPHERE;

typedef struct PG_DLL _tagParticleCBuffer
{
	Vector3		vCamAxisX;
	float		fEmpty1;
	Vector3		vCamAxisY;
...
typedef struct PG_DLL _tagLandScapeCBuffer
{
	int		iDetailLevel;
	int		iSplatCount;
	float   fRangeBrush;
